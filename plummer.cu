...
int main(const int argc, const char **argv) {
  ...
  for (int i = 0; i < nstep; i++) {
    ...
    leapint<<<num_blocks, BLOCK_SIZE>>>(device_particles.p, device_particles.v,
                                        dt);
    cudaDeviceSynchronize();
    integrate_position<<<num_blocks, BLOCK_SIZE>>>(device_particles.p,
                                                   device_particles.v, dt);
    cudaDeviceSynchronize();
    leapint<<<num_blocks, BLOCK_SIZE>>>(device_particles.p, device_particles.v,
                                        dt);
    cudaDeviceSynchronize();
...
double unirandom() { return ((double)rand()) / RAND_MAX; }
...
__global__ void integrate_position(vector3 *p, vector3 *v, double dt) {
  ...
  if (i < NUM_PARTICLES) {
    p[i] = (vector3){p[i].x + v[i].x * dt, p[i].y + v[i].y * dt,
                     p[i].z + v[i].z * dt};
  }
}
...
__global__ void leapint(vector3 *p, vector3 *v, double dt) {
  ...
  if (i < NUM_PARTICLES) {
    ...
    v[i] = (vector3){v[i].x + 0.5 * GM * dt * ax, v[i].y + 0.5 * GM * dt * ay,
                     v[i].z + 0.5 * GM * dt * az};
  }
}
